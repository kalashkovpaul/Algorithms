// 7_1. MSD для строк.
// Дан массив строк. Количество строк не больше 105. Отсортировать массив методом поразрядной \
...
void msdSort(string strings[], int start, int end, int digit, int R) {
    ...
    delete[] auxStrings;
    for (int i = 0; i < countersSize - 1; i++) {
        if (oldCounters[i] != oldCounters[i + 1] && oldCounters[i] < oldCounters[i + 1] - 1)
            msdSort(strings, oldCounters[i], oldCounters[i + 1], digit + 1, R);
    }
    delete[] counters;
    delete[] oldCounters;
}
