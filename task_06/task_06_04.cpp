...
size_t partition (T *data, size_t start, size_t end, SortComparator comp=SortComparator()) {
    int pivot = start + rand() % (end - start);
    std::swap(data[pivot], data[end - 1]);
    size_t i = start;
    while (i < end - 1 && comp(data[i], data[end - 1]))
        i++;
    size_t j = i;
    while (j < end - 1 && comp(data[end - 1], data[j]))
        j++;
    while (j != end - 1) {
...
template<class T, class SortComparator=DefaultComparator<T>>
void kthElement(T *data, size_t size, size_t k) {
    int l = 0;
    int r = size;
    
    while (true) {
        int m = partition<T, SortComparator>(data, l, r);
        if (m == k) {
            return;
        }
        else if (m < k) {
