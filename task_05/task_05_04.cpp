...
struct DotComparator {
    bool operator() (const dot_t& l, const dot_t& r) {
...
void merge(T array[], size_t left, size_t middle, size_t right)
{
	...
	for (size_t i = 0; i < leftSubArray; i++)
	...
	while (indexForLeftSubArray < leftSubArray && indexForRigthSubArray < rigthSubArray) {
		if (comp(leftArray[indexForLeftSubArray], rightArray[indexForRigthSubArray])) {
...
int main()
{
    ...
    for (int i = 0; i < dotsAmount - 1; i++) {
        ...
        if (thickness == 1)
            length += (dots[i + 1].value - dots[i].value);
    }
