...
void findIntersection(array_t *firstArray, array_t *secondArray, array_t *resultArray) {
    int leftBinary = 0, rightBinary = 0;
    int index = 0;
    resultArray->length = 0;
    findLimitsForBinary(firstArray, secondArray->array[0], &leftBinary, &rightBinary);
    index = binarySearch(firstArray, leftBinary, rightBinary, secondArray->array[0]);
    if (index != NOT_FOUND) {
    ...
    i < secondArray->length) {
        ...
        while (high - 1 < firstArray->length && firstArray->array[high - 1] <= secondArray->array[i])
            high *= 2;
        ...
        index = binarySearch(firstArray, leftBinary, rightBinary, secondArray->array[i]);
        if (index != NOT_FOUND) {
